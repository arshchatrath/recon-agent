...
rather than to make decisions forever.
...
(2) is essential. It needs no knowledge of fees, so it exists from batch 1, without it nothing can be matched until a fee rule is promoted, and no fee rule
...
| Ambiguity is never guessed | subset-sum escalates when truncated or tied; the assignment solver may decline |
